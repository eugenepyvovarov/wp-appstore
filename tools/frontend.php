...
@set_time_limit( 360 );
if (! class_exists('WP_AppStore') ){
    $inc = WP_PLUGIN_DIR."/wp-appstore/wp-appstore.class.php";
    if ( file_exists( $inc ) )
		include_once $inc;
}
        ...
        if ($id = post_exists($plugin->title)) {
            $postmeta = get_post_custom($id);
                if (!isset($postmeta['version']) || $postmeta['version'] != $plugin->version) {
                    ...
                    $post_id = updatePost($id, $plugin->title, $plugin->description.$scr, $plugin->slug, null, $category, 'publish', 'wp-appstore-plugin', $meta);
            ...
            $post_id = insertPost($plugin->title, $plugin->description.$scr, $plugin->slug, null, $category, 'publish', 'wp-appstore-plugin', $author_id[0], true, 'open', $meta);
            if (is_array($plugin->tags)) {
                wp_set_post_tags($post_id, implode(',', $plugin->tags));
            }
        }
        ...
        if ($id = post_exists($theme->title)) {
            $postmeta = get_post_custom($id);
                if (!isset($postmeta['version']) || $postmeta['version'] != $theme->version) {
                    ...
                    $post_id = updatePost($id, $theme->title, $theme->description.$scr, $theme->slug, null, $category, 'publish', 'wp-appstore-theme', $meta);
            ...
            $post_id = insertPost($theme->title, $theme->description.$scr, $theme->slug, null, $category, 'publish', 'wp-appstore-theme', $author_id[0], true, 'open', $meta);
            if (is_array($theme->tags)) {
                wp_set_post_tags($post_id, implode(',', $theme->tags));
            }
        }
