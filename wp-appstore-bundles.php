...
function wp_appstore_wizard_bundles() {
    ...
    if(is_array($bundles))
        ...
        for ($i=0;$i < sizeof($bundles); $i++) {
            foreach ($bundles as $bundle_slug => $bundle) {
                if ($bundle['order'] == $i) {
                    $list .= '<li>';
                    $list .= '<a href="'.esc_attr(WP_AppStore::admin_url(array('screen'=>'show-bundle', 'bundle' => $bundle_slug))).'">'.$bundle['name'].'</a>';
                    $list .= '</li>';
                }
            }
        }
...
function wp_appstore_wizard_show_bundle($bundle_slug) {
    $bundles = get_site_transient('wp_appstore_bundles');
    $list = '<ul>';
    if(is_array($bundles) && array_key_exists($bundle_slug, $bundles)){
        $bundle = $bundles[$bundle_slug];
    ...
    ?>
     <div class="wrap">
        ...
        <input type="hidden" name="prepared-bundle" value="<?php echo $bundle_slug; ?>" />
            ...
            <ul>
                <?php foreach($bundle['plugins'] as $plugin): ?>
                    <li><span class="plugin-data"><?php echo $plugin['title'] ?></span></li>
                <?php endforeach; ?>
            </ul>
            <?php endif; ?>
            <?php if(is_array($bundle['themes'])): ?>
            ...
            <ul>
                <?php foreach($bundle['themes'] as $theme): ?>
                    <li><span class="plugin-data"><?php echo $theme['title'] ?></span></li>
                <?php endforeach; ?>
            </ul>
            <?php endif; ?>
        <?php endif; ?>
        <span class="buyoptions"><a href="<?php echo esc_attr(WP_AppStore::admin_url(array('prepared-bundle'=>$bundle_slug, 'status'=>'rfc')));?>" class="button rbutton" title="Install Bundle">Install this Bundle</a></span>
        <span class="buyoptions"><a href="<?php echo esc_attr(WP_AppStore::admin_url(array('screen'=>'prepare-bundle', 'bundle'=>$bundle_slug)));?>" class="button rbutton" title="Modify Bundle">Modify Bundle</a></span>
        <span class="buyoptions"><a href="<?php echo esc_attr(WP_AppStore::admin_url(array('screen'=>'wizard')));?>" class="button rbutton" title="Bundles Wizard">Go back to the bundles</a></span>
    </div>
    <?php
}
